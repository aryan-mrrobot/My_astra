...
class Trikuta:
    ...
    def detect_xss(self, payload, injected_link):
        ...
        try:
            partial_score = fuzz.token_set_ratio(payload.lower(), self.browser.html.lower())
    ...
    def test_xss(self):
        ...
        for payload in payloads:
            print()
            self.inject_payload(payload, self.user_args.URL, self.user_args.REQUEST_DELAY)
    ...
    def parse_args(self):
        ...
        parser.add_argument('-u', action='store', dest='URL', help='The URL to inject XSS payloads into.', required=True)
        ...
        if "{xss}" not in arguments.URL:
            print(Color.RED + "Please provide the '{xss}' placeholder for injection point in the URL" + Color.END)
            print(Color.GREEN + "Example: -u \"http://example.com/index.php?name={xss}\"" + Color.END)
            sys.exit()
